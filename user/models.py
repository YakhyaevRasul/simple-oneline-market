...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    phone_number = models.CharField(
        ...
        unique=True,
        help_text=_("998901234567 holatda kiriting"),
    )
    USERNAME_FIELD = "phone_number"

    # additional fields
    firstname = models.CharField(_("Ismi:"), max_length=150, null=True, blank=True)
    lastname = models.CharField(_("Familyasi:"), max_length=150, null=True, blank=True)
    ...
    liked_products = models.ManyToManyField(
        "product.Product", blank=True, related_name="liked_products"
    )
...
def _expire_at_default():
    return timezone.now() + timezone.timedelta(
        minutes=settings.CODE_VERIFICATION_EXPIRE_TIME
    )
...
class VerificationCode(models.Model):
    ...
    contact = models.CharField(
        _("Aloqa raqami"), max_length=255, null=False, blank=False
    )
